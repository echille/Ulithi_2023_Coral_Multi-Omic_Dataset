...
filepath <- "~/Downloads/3_Raw_proteomic_data_all_genera/2-Acropora" #Path to intensity matrix. This is saved into multiple chunked gzipped files that will have to be combined below.
rdat <- read_delim("2-Proteomic_workflows/2-Acropora/1-Input/Acropora_proteomic_run_metadata.csv", delim = ",") #Load run metadata, matrix containing run IDs and associated sample IDs
sdat <- read_delim("2-Proteomic_workflows/2-Acropora/1-Input/Acropora_sample_metadata.csv", delim = ",") #Load sample metadata
...
pdat <- filter(pdat, grepl("ECL", Run))
...
dat <- filter(dat, !is.na(Protein.Group))

dat.host <- dat %>% filter(grepl("Acropora_", Protein.Group))
n_distinct(dat.host$Protein.Group)
n_distinct(dat.host$sample_id)

save(dat.host, file = "2-Proteomic_workflows/2-Acropora/3-Output/Ulithi23_Acropora_host_proteomic_raw_dataset")
