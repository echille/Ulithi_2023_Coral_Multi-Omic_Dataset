...
filepath <- "~/Downloads/3_Raw_proteomic_data_all_genera/1_Pocillopora" #Path to intensity matrix. This is saved into multiple chunked gzipped files that will have to be combined below.
rdat <- read_delim("2-Proteomic_workflows/3-Pocillopora/1-Input/Pocillopora_proteomic_run_metadata.csv", delim = ",") #Load run metadata, matrix containing run IDs and associated sample IDs
sdat <- read_delim("2-Proteomic_workflows/3-Pocillopora/1-Input/Pocillopora_sample_metadata.csv", delim = ",") #Load sample metadata
...
dat.host <- dat %>% filter(grepl("Pocillopora_", Protein.Group))
n_distinct(dat.host$Protein.Group)
n_distinct(dat.host$sample_id)

save(dat.host, file = "2-Proteomic_workflows/3-Pocillopora/3-Output/Ulithi23_Pocillopora_host_proteomic_raw_dataset")
